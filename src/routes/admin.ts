...
import { forbiddenResponse, badRequestResponse, notFoundResponse, createdResponse, conflictResponse } from '@/utils/http';

function checkMasterKey(headers: Record<string, string | undefined>): Response | undefined {
  const key = headers['x-master-key'];
  if (!key) return forbiddenResponse();
  const a = Buffer.from(key);
  const b = Buffer.from(config.masterKey);
  if (a.length !== b.length || !timingSafeEqual(a, b)) return forbiddenResponse();
}
